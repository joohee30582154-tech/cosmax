...
def build_html(_html_mtime: float) -> tuple[str, list[str]]:
    """index.html을 읽어서 image/nail-XX.jpg 참조를 base64 데이터로 치환한다.

    _html_mtime을 캐시 키에 포함시켜, index.html이 수정되면 캐시가 자동으로
    무효화되고 최신 내용으로 다시 빌드되도록 한다.
    """
...
html_content, missing_images = build_html(HTML_PATH.stat().st_mtime)
